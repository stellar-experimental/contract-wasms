(module
  (type (;0;) (func (param i64 i64) (result i64)))
  (type (;1;) (func (param i64) (result i64)))
  (type (;2;) (func (result i64)))
  (type (;3;) (func (param i64 i64 i64) (result i64)))
  (type (;4;) (func (param i32 i64)))
  (type (;5;) (func (param i64 i64) (result i32)))
  (type (;6;) (func (param i64 i64 i64 i64) (result i64)))
  (type (;7;) (func (param i32)))
  (type (;8;) (func (param i64)))
  (type (;9;) (func (param i64 i64)))
  (type (;10;) (func (param i64) (result i32)))
  (type (;11;) (func))
  (type (;12;) (func (param i32 i32) (result i64)))
  (type (;13;) (func (param i32 i32 i32)))
  (type (;14;) (func (param i32 i64 i64)))
  (type (;15;) (func (param i32) (result i64)))
  (type (;16;) (func (param i32 i64 i64 i64)))
  (type (;17;) (func (param i64 i64 i64) (result i32)))
  (type (;18;) (func (result i32)))
  (type (;19;) (func (param i32 i32 i32 i32) (result i64)))
  (type (;20;) (func (param i64 i64 i64 i64 i64) (result i64)))
  (import "l" "1" (func (;0;) (type 0)))
  (import "l" "2" (func (;1;) (type 0)))
  (import "v" "_" (func (;2;) (type 2)))
  (import "v" "3" (func (;3;) (type 1)))
  (import "v" "1" (func (;4;) (type 0)))
  (import "v" "6" (func (;5;) (type 0)))
  (import "l" "_" (func (;6;) (type 3)))
  (import "l" "7" (func (;7;) (type 6)))
  (import "x" "1" (func (;8;) (type 0)))
  (import "a" "0" (func (;9;) (type 1)))
  (import "x" "7" (func (;10;) (type 2)))
  (import "a" "3" (func (;11;) (type 1)))
  (import "d" "0" (func (;12;) (type 3)))
  (import "d" "_" (func (;13;) (type 3)))
  (import "m" "_" (func (;14;) (type 2)))
  (import "m" "0" (func (;15;) (type 3)))
  (import "b" "8" (func (;16;) (type 1)))
  (import "l" "6" (func (;17;) (type 1)))
  (import "i" "6" (func (;18;) (type 0)))
  (import "i" "3" (func (;19;) (type 0)))
  (import "i" "5" (func (;20;) (type 1)))
  (import "i" "4" (func (;21;) (type 1)))
  (import "v" "g" (func (;22;) (type 0)))
  (import "m" "9" (func (;23;) (type 3)))
  (import "i" "8" (func (;24;) (type 1)))
  (import "i" "7" (func (;25;) (type 1)))
  (import "b" "j" (func (;26;) (type 0)))
  (import "l" "0" (func (;27;) (type 0)))
  (import "l" "8" (func (;28;) (type 0)))
  (import "x" "0" (func (;29;) (type 0)))
  (import "x" "5" (func (;30;) (type 1)))
  (memory (;0;) 17)
  (global (;0;) (mut i32) i32.const 1048576)
  (global (;1;) i32 i32.const 1048912)
  (global (;2;) i32 i32.const 1049072)
  (global (;3;) i32 i32.const 1049072)
  (export "memory" (memory 0))
  (export "__constructor" (func 56))
  (export "accept_admin" (func 58))
  (export "cancel_admin_proposal" (func 65))
  (export "claim_rewards" (func 67))
  (export "deposit" (func 69))
  (export "get_admin" (func 72))
  (export "get_all_balances" (func 73))
  (export "get_balance" (func 74))
  (export "get_pending_admin" (func 75))
  (export "propose_admin" (func 76))
  (export "sync_balance" (func 78))
  (export "upgrade" (func 79))
  (export "withdraw" (func 80))
  (export "_" (global 1))
  (export "__data_end" (global 2))
  (export "__heap_base" (global 3))
  (func (;31;) (type 16) (param i32 i64 i64 i64)
    (local i32 i32 i32 i64 i64)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 5
    global.set 0
    local.get 5
    local.get 1
    call 32
    i32.const 1
    local.set 4
    block ;; label = @1
      local.get 2
      local.get 5
      i64.load
      local.tee 7
      i64.add
      local.tee 8
      local.get 7
      i64.lt_u
      local.tee 6
      local.get 6
      i64.extend_i32_u
      local.get 3
      local.get 5
      i64.load offset=8
      local.tee 7
      i64.add
      i64.add
      local.tee 2
      local.get 7
      i64.lt_u
      local.get 2
      local.get 7
      i64.eq
      select
      i32.const 1
      i32.eq
      if ;; label = @2
        local.get 0
        i32.const 4
        i32.store offset=4
        br 1 (;@1;)
      end
      local.get 1
      local.get 8
      local.get 2
      call 33
      local.tee 4
      if ;; label = @2
        local.get 0
        local.get 4
        i32.store offset=4
        i32.const 1
        local.set 4
        br 1 (;@1;)
      end
      local.get 0
      local.get 8
      i64.store offset=16
      local.get 0
      local.get 2
      i64.store offset=24
      i32.const 0
      local.set 4
    end
    local.get 0
    local.get 4
    i32.store
    local.get 5
    i32.const 16
    i32.add
    global.set 0
  )
  (func (;32;) (type 4) (param i32 i64)
    (local i32 i64)
    global.get 0
    i32.const 32
    i32.sub
    local.tee 2
    global.set 0
    i64.const 0
    local.get 1
    call 34
    if ;; label = @1
      i64.const 0
      local.get 1
      call 35
    end
    block ;; label = @1
      local.get 0
      i64.const 0
      local.get 1
      call 36
      local.tee 1
      i64.const 1
      call 37
      if (result i64) ;; label = @2
        local.get 2
        local.get 1
        i64.const 1
        call 0
        call 38
        local.get 2
        i32.load
        i32.const 1
        i32.eq
        br_if 1 (;@1;)
        local.get 2
        i64.load offset=24
        local.set 3
        local.get 2
        i64.load offset=16
      else
        i64.const 0
      end
      i64.store
      local.get 0
      local.get 3
      i64.store offset=8
      local.get 2
      i32.const 32
      i32.add
      global.set 0
      return
    end
    unreachable
  )
  (func (;33;) (type 17) (param i64 i64 i64) (result i32)
    (local i32 i32 i32 i32 i64 i64 i64)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 3
    global.set 0
    block ;; label = @1
      block ;; label = @2
        block ;; label = @3
          block ;; label = @4
            block ;; label = @5
              local.get 1
              local.get 2
              i64.or
              i64.eqz
              if ;; label = @6
                i64.const 0
                local.get 0
                call 34
                if ;; label = @7
                  i64.const 0
                  local.get 0
                  call 36
                  i64.const 1
                  call 1
                  drop
                end
                i64.const 1
                local.get 2
                call 34
                i32.eqz
                br_if 5 (;@1;)
                i64.const 1
                local.get 2
                call 35
                local.get 3
                call 39
                local.get 3
                i32.load
                i32.const 1
                i32.ne
                br_if 5 (;@1;)
                local.get 3
                i64.load offset=8
                local.set 1
                call 2
                local.set 2
                local.get 1
                call 3
                i64.const 32
                i64.shr_u
                i32.wrap_i64
                local.set 5
                loop ;; label = @7
                  local.get 5
                  local.get 6
                  local.tee 4
                  i32.eq
                  br_if 2 (;@5;)
                  local.get 4
                  i32.const 1
                  i32.add
                  local.set 6
                  local.get 4
                  local.get 1
                  call 3
                  i64.const 32
                  i64.shr_u
                  i32.wrap_i64
                  i32.ge_u
                  br_if 0 (;@7;)
                  local.get 1
                  local.get 4
                  i64.extend_i32_u
                  i64.const 32
                  i64.shl
                  i64.const 4
                  i64.or
                  call 4
                  local.tee 7
                  i64.const 255
                  i64.and
                  i64.const 77
                  i64.ne
                  br_if 4 (;@3;)
                  local.get 7
                  local.get 0
                  call 40
                  br_if 0 (;@7;)
                  local.get 2
                  local.get 7
                  call 5
                  local.set 2
                  br 0 (;@7;)
                end
                unreachable
              end
              i64.const 0
              local.get 0
              call 36
              local.get 1
              local.get 2
              call 41
              i64.const 1
              call 6
              drop
              i64.const 0
              local.get 0
              call 35
              block (result i64) ;; label = @6
                i64.const 1
                local.get 2
                call 34
                i32.eqz
                if ;; label = @7
                  call 2
                  br 1 (;@6;)
                end
                i64.const 1
                local.get 2
                call 35
                local.get 3
                call 39
                local.get 3
                i32.load
                i32.const 1
                i32.ne
                br_if 4 (;@2;)
                local.get 3
                i64.load offset=8
              end
              local.tee 7
              call 3
              i64.const 32
              i64.shr_u
              local.set 8
              i64.const 0
              local.set 2
              i64.const 4
              local.set 1
              loop ;; label = @6
                block ;; label = @7
                  local.get 2
                  local.get 8
                  i64.ne
                  if ;; label = @8
                    local.get 2
                    local.get 7
                    call 3
                    i64.const 32
                    i64.shr_u
                    i64.ge_u
                    br_if 6 (;@2;)
                    local.get 7
                    local.get 1
                    call 4
                    local.tee 9
                    i64.const 255
                    i64.and
                    i64.const 77
                    i64.eq
                    br_if 1 (;@7;)
                    br 5 (;@3;)
                  end
                  local.get 7
                  local.get 0
                  call 5
                  call 42
                  i64.const 1
                  local.get 2
                  call 35
                  br 3 (;@4;)
                end
                local.get 2
                i64.const 1
                i64.add
                local.set 2
                local.get 1
                i64.const 4294967296
                i64.add
                local.set 1
                local.get 9
                local.get 0
                call 40
                i32.eqz
                br_if 0 (;@6;)
              end
              br 1 (;@4;)
            end
            local.get 2
            call 3
            i64.const 4294967295
            i64.le_u
            if ;; label = @5
              i64.const 1
              local.get 1
              call 36
              i64.const 1
              call 1
              drop
              br 1 (;@4;)
            end
            local.get 2
            call 42
            i64.const 1
            local.get 1
            call 35
          end
          i32.const 0
          local.set 4
          br 2 (;@1;)
        end
        unreachable
      end
      i32.const 10
      local.set 4
    end
    local.get 3
    i32.const 16
    i32.add
    global.set 0
    local.get 4
  )
  (func (;34;) (type 5) (param i64 i64) (result i32)
    local.get 0
    local.get 1
    call 36
    i64.const 1
    call 37
  )
  (func (;35;) (type 9) (param i64 i64)
    local.get 0
    local.get 1
    call 36
    i64.const 1
    i64.const 2226511046246404
    i64.const 27089217729331204
    call 7
    drop
  )
  (func (;36;) (type 0) (param i64 i64) (result i64)
    (local i32)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 2
    global.set 0
    block ;; label = @1
      block ;; label = @2
        block ;; label = @3
          local.get 0
          i32.wrap_i64
          i32.const 1
          i32.and
          if ;; label = @4
            local.get 2
            i32.const 1048678
            i32.const 9
            call 50
            local.get 2
            i32.load
            br_if 2 (;@2;)
            local.get 2
            local.get 2
            i64.load offset=8
            call 51
            br 1 (;@3;)
          end
          local.get 2
          i32.const 1048671
          i32.const 7
          call 50
          local.get 2
          i32.load
          br_if 1 (;@2;)
          local.get 2
          local.get 2
          i64.load offset=8
          local.get 1
          call 52
        end
        local.get 2
        i64.load offset=8
        local.set 0
        local.get 2
        i64.load
        i64.eqz
        br_if 1 (;@1;)
      end
      unreachable
    end
    local.get 2
    i32.const 16
    i32.add
    global.set 0
    local.get 0
  )
  (func (;37;) (type 5) (param i64 i64) (result i32)
    local.get 0
    local.get 1
    call 27
    i64.const 1
    i64.eq
  )
  (func (;38;) (type 4) (param i32 i64)
    (local i32 i64)
    local.get 0
    block (result i64) ;; label = @1
      block ;; label = @2
        local.get 1
        i32.wrap_i64
        i32.const 255
        i32.and
        local.tee 2
        i32.const 68
        i32.ne
        if ;; label = @3
          local.get 2
          i32.const 10
          i32.ne
          br_if 1 (;@2;)
          local.get 0
          i64.const 0
          i64.store offset=24
          local.get 0
          local.get 1
          i64.const 8
          i64.shr_u
          i64.store offset=16
          i64.const 0
          br 2 (;@1;)
        end
        local.get 1
        call 20
        local.set 3
        local.get 1
        call 21
        local.set 1
        local.get 0
        local.get 3
        i64.store offset=24
        local.get 0
        local.get 1
        i64.store offset=16
        i64.const 0
        br 1 (;@1;)
      end
      local.get 0
      i64.const 34359740419
      i64.store offset=8
      i64.const 1
    end
    i64.store
  )
  (func (;39;) (type 7) (param i32)
    (local i64)
    block ;; label = @1
      local.get 0
      i64.const 1
      i64.const 0
      call 36
      local.tee 1
      i64.const 1
      call 37
      if (result i64) ;; label = @2
        local.get 1
        i64.const 1
        call 0
        local.tee 1
        i64.const 255
        i64.and
        i64.const 75
        i64.ne
        br_if 1 (;@1;)
        local.get 0
        local.get 1
        i64.store offset=8
        i64.const 1
      else
        i64.const 0
      end
      i64.store
      return
    end
    unreachable
  )
  (func (;40;) (type 5) (param i64 i64) (result i32)
    local.get 0
    local.get 1
    call 29
    i64.eqz
  )
  (func (;41;) (type 0) (param i64 i64) (result i64)
    (local i32)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 2
    global.set 0
    local.get 2
    local.get 0
    local.get 1
    call 54
    local.get 2
    i32.load
    i32.const 1
    i32.eq
    if ;; label = @1
      unreachable
    end
    local.get 2
    i64.load offset=8
    local.get 2
    i32.const 16
    i32.add
    global.set 0
  )
  (func (;42;) (type 8) (param i64)
    i64.const 1
    local.get 0
    call 36
    local.get 0
    i64.const 1
    call 6
    drop
  )
  (func (;43;) (type 10) (param i64) (result i32)
    i32.const 3
    i32.const 0
    local.get 0
    call 44
    select
  )
  (func (;44;) (type 10) (param i64) (result i32)
    (local i32 i32)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 1
    global.set 0
    local.get 1
    call 61
    block (result i32) ;; label = @1
      local.get 1
      i32.load
      i32.const 1
      i32.eq
      if ;; label = @2
        local.get 1
        i32.load offset=4
        br 1 (;@1;)
      end
      local.get 0
      local.get 1
      i64.load offset=8
      call 60
    end
    local.get 1
    i32.const 16
    i32.add
    global.set 0
  )
  (func (;45;) (type 18) (result i32)
    (local i32)
    call 46
    i64.const 2
    call 37
    local.tee 0
    if ;; label = @1
      call 47
    end
    local.get 0
  )
  (func (;46;) (type 2) (result i64)
    (local i32 i64)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 0
    global.set 0
    local.get 0
    i32.const 1048660
    i32.const 11
    call 50
    block ;; label = @1
      local.get 0
      i32.load
      i32.eqz
      if ;; label = @2
        local.get 0
        local.get 0
        i64.load offset=8
        call 51
        local.get 0
        i32.load
        i32.const 1
        i32.ne
        br_if 1 (;@1;)
      end
      unreachable
    end
    local.get 0
    i64.load offset=8
    local.get 0
    i32.const 16
    i32.add
    global.set 0
  )
  (func (;47;) (type 11)
    i64.const 2226511046246404
    i64.const 27089217729331204
    call 28
    drop
  )
  (func (;48;) (type 0) (param i64 i64) (result i64)
    (local i32 i32)
    global.get 0
    i32.const 32
    i32.sub
    local.tee 3
    global.set 0
    local.get 3
    local.get 1
    i64.store offset=8
    local.get 3
    local.get 0
    i64.store
    loop (result i64) ;; label = @1
      local.get 2
      i32.const 16
      i32.eq
      if (result i64) ;; label = @2
        i32.const 0
        local.set 2
        loop ;; label = @3
          local.get 2
          i32.const 16
          i32.ne
          if ;; label = @4
            local.get 3
            i32.const 16
            i32.add
            local.get 2
            i32.add
            local.get 2
            local.get 3
            i32.add
            i64.load
            i64.store
            local.get 2
            i32.const 8
            i32.add
            local.set 2
            br 1 (;@3;)
          end
        end
        local.get 3
        i32.const 16
        i32.add
        i32.const 2
        call 49
        local.get 3
        i32.const 32
        i32.add
        global.set 0
      else
        local.get 3
        i32.const 16
        i32.add
        local.get 2
        i32.add
        i64.const 2
        i64.store
        local.get 2
        i32.const 8
        i32.add
        local.set 2
        br 1 (;@1;)
      end
    end
  )
  (func (;49;) (type 12) (param i32 i32) (result i64)
    local.get 0
    i64.extend_i32_u
    i64.const 32
    i64.shl
    i64.const 4
    i64.or
    local.get 1
    i64.extend_i32_u
    i64.const 32
    i64.shl
    i64.const 4
    i64.or
    call 22
  )
  (func (;50;) (type 13) (param i32 i32 i32)
    (local i32)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 3
    global.set 0
    local.get 3
    local.get 1
    local.get 2
    call 82
    local.get 0
    local.get 3
    i32.load
    if (result i64) ;; label = @1
      i64.const 1
    else
      local.get 0
      local.get 3
      i64.load offset=8
      i64.store offset=8
      i64.const 0
    end
    i64.store
    local.get 3
    i32.const 16
    i32.add
    global.set 0
  )
  (func (;51;) (type 4) (param i32 i64)
    (local i32)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 2
    global.set 0
    local.get 2
    local.get 1
    i64.store offset=8
    local.get 2
    i32.const 8
    i32.add
    i32.const 1
    call 49
    local.set 1
    local.get 0
    i64.const 0
    i64.store
    local.get 0
    local.get 1
    i64.store offset=8
    local.get 2
    i32.const 16
    i32.add
    global.set 0
  )
  (func (;52;) (type 14) (param i32 i64 i64)
    (local i32)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 3
    global.set 0
    local.get 3
    local.get 2
    i64.store offset=8
    local.get 3
    local.get 1
    i64.store
    local.get 3
    i32.const 2
    call 49
    local.set 1
    local.get 0
    i64.const 0
    i64.store
    local.get 0
    local.get 1
    i64.store offset=8
    local.get 3
    i32.const 16
    i32.add
    global.set 0
  )
  (func (;53;) (type 15) (param i32) (result i64)
    (local i32 i64)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 1
    global.set 0
    block ;; label = @1
      block (result i64) ;; label = @2
        local.get 0
        i32.load
        i32.const 1
        i32.eq
        if ;; label = @3
          local.get 0
          i32.load offset=4
          i32.const 1
          i32.sub
          i64.extend_i32_u
          i64.const 32
          i64.shl
          i64.const 4294967299
          i64.add
          br 1 (;@2;)
        end
        local.get 1
        local.get 0
        i64.load offset=16
        local.get 0
        i64.load offset=24
        call 54
        local.get 1
        i32.load
        i32.const 1
        i32.eq
        br_if 1 (;@1;)
        local.get 1
        i64.load offset=8
      end
      local.get 1
      i32.const 16
      i32.add
      global.set 0
      return
    end
    unreachable
  )
  (func (;54;) (type 14) (param i32 i64 i64)
    local.get 1
    i64.const 72057594037927935
    i64.gt_u
    local.get 2
    i64.const 0
    i64.ne
    local.get 2
    i64.eqz
    select
    if (result i64) ;; label = @1
      local.get 2
      local.get 1
      call 19
    else
      local.get 1
      i64.const 8
      i64.shl
      i64.const 10
      i64.or
    end
    local.set 1
    local.get 0
    i64.const 0
    i64.store
    local.get 0
    local.get 1
    i64.store offset=8
  )
  (func (;55;) (type 15) (param i32) (result i64)
    local.get 0
    i32.load
    i32.eqz
    if ;; label = @1
      local.get 0
      i64.load offset=8
      return
    end
    local.get 0
    i32.load offset=4
    i32.const 1
    i32.sub
    i64.extend_i32_u
    i64.const 32
    i64.shl
    i64.const 4294967299
    i64.add
  )
  (func (;56;) (type 1) (param i64) (result i64)
    (local i32 i32)
    global.get 0
    i32.const 32
    i32.sub
    local.tee 2
    global.set 0
    local.get 0
    i64.const 255
    i64.and
    i64.const 77
    i64.eq
    if ;; label = @1
      local.get 0
      call 57
      call 46
      i64.const 1
      i64.const 2
      call 6
      drop
      call 47
      local.get 2
      i64.const 65155576823152142
      i64.store offset=8
      local.get 2
      i64.const 3141253390
      i64.store
      loop ;; label = @2
        local.get 1
        i32.const 16
        i32.eq
        if ;; label = @3
          i32.const 0
          local.set 1
          loop ;; label = @4
            local.get 1
            i32.const 16
            i32.ne
            if ;; label = @5
              local.get 2
              i32.const 16
              i32.add
              local.get 1
              i32.add
              local.get 1
              local.get 2
              i32.add
              i64.load
              i64.store
              local.get 1
              i32.const 8
              i32.add
              local.set 1
              br 1 (;@4;)
            end
          end
          local.get 2
          i32.const 16
          i32.add
          i32.const 2
          call 49
          local.get 0
          call 8
          drop
          local.get 2
          i32.const 32
          i32.add
          global.set 0
          i64.const 2
          return
        else
          local.get 2
          i32.const 16
          i32.add
          local.get 1
          i32.add
          i64.const 2
          i64.store
          local.get 1
          i32.const 8
          i32.add
          local.set 1
          br 1 (;@2;)
        end
        unreachable
      end
      unreachable
    end
    unreachable
  )
  (func (;57;) (type 8) (param i64)
    i64.const 52571740430
    local.get 0
    call 77
    call 47
  )
  (func (;58;) (type 1) (param i64) (result i64)
    (local i32 i64 i64)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 1
    global.set 0
    block (result i64) ;; label = @1
      block ;; label = @2
        local.get 0
        i64.const 255
        i64.and
        i64.const 77
        i64.eq
        if ;; label = @3
          call 45
          if (result i32) ;; label = @4
            local.get 1
            call 59
            block (result i32) ;; label = @5
              local.get 1
              i32.load
              i32.const 1
              i32.ne
              if ;; label = @6
                i32.const 4
                local.get 0
                local.get 1
                i64.load offset=8
                call 60
                br_if 1 (;@5;)
                drop
                local.get 0
                call 9
                drop
                local.get 1
                call 61
                local.get 1
                i32.load
                i32.const 1
                i32.ne
                br_if 4 (;@2;)
              end
              local.get 1
              i32.load offset=4
            end
            i32.const 2
            i32.shl
            i32.load offset=1048708
          else
            i32.const 1
          end
          i32.const 1
          i32.sub
          i64.extend_i32_u
          i64.const 32
          i64.shl
          i64.const 4294967299
          i64.add
          br 2 (;@1;)
        end
        unreachable
      end
      local.get 1
      i64.load offset=8
      local.set 2
      local.get 0
      call 57
      call 62
      i64.const 679987919857678
      call 63
      local.get 1
      local.get 2
      i64.store offset=8
      local.get 1
      local.get 0
      i64.store
      i32.const 1048808
      i32.const 2
      local.get 1
      i32.const 2
      call 64
      call 8
      drop
      i64.const 2
    end
    local.get 1
    i32.const 16
    i32.add
    global.set 0
  )
  (func (;59;) (type 7) (param i32)
    (local i32)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 1
    global.set 0
    local.get 1
    i64.const 483902301132814
    call 81
    local.get 0
    block (result i32) ;; label = @1
      local.get 1
      i32.load
      i32.eqz
      if ;; label = @2
        local.get 0
        i32.const 3
        i32.store offset=4
        i32.const 1
        br 1 (;@1;)
      end
      local.get 0
      local.get 1
      i64.load offset=8
      i64.store offset=8
      call 47
      i32.const 0
    end
    i32.store
    local.get 1
    i32.const 16
    i32.add
    global.set 0
  )
  (func (;60;) (type 5) (param i64 i64) (result i32)
    local.get 0
    local.get 1
    call 40
    i32.const 1
    i32.xor
  )
  (func (;61;) (type 7) (param i32)
    (local i32 i64)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 1
    global.set 0
    local.get 1
    i64.const 52571740430
    call 81
    local.get 0
    block (result i32) ;; label = @1
      local.get 1
      i32.load
      i32.eqz
      if ;; label = @2
        local.get 0
        i32.const 1
        i32.store offset=4
        i32.const 1
        br 1 (;@1;)
      end
      local.get 1
      i64.load offset=8
      local.set 2
      call 47
      local.get 0
      local.get 2
      i64.store offset=8
      i32.const 0
    end
    i32.store
    local.get 1
    i32.const 16
    i32.add
    global.set 0
  )
  (func (;62;) (type 11)
    i64.const 483902301132814
    i64.const 2
    call 37
    if ;; label = @1
      i64.const 483902301132814
      i64.const 2
      call 1
      drop
    end
  )
  (func (;63;) (type 1) (param i64) (result i64)
    (local i32 i32 i64)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 1
    global.set 0
    local.get 1
    local.get 0
    i64.store
    i64.const 2
    local.set 3
    i32.const 1
    local.set 2
    loop ;; label = @1
      local.get 2
      if ;; label = @2
        local.get 2
        i32.const 1
        i32.sub
        local.set 2
        local.get 0
        local.set 3
        br 1 (;@1;)
      end
    end
    local.get 1
    local.get 3
    i64.store offset=8
    local.get 1
    i32.const 8
    i32.add
    i32.const 1
    call 49
    local.get 1
    i32.const 16
    i32.add
    global.set 0
  )
  (func (;64;) (type 19) (param i32 i32 i32 i32) (result i64)
    local.get 1
    local.get 3
    i32.ne
    if ;; label = @1
      unreachable
    end
    local.get 0
    i64.extend_i32_u
    i64.const 32
    i64.shl
    i64.const 4
    i64.or
    local.get 2
    i64.extend_i32_u
    i64.const 32
    i64.shl
    i64.const 4
    i64.or
    local.get 1
    i64.extend_i32_u
    i64.const 32
    i64.shl
    i64.const 4
    i64.or
    call 23
  )
  (func (;65;) (type 1) (param i64) (result i64)
    (local i32 i32 i64)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 1
    global.set 0
    block (result i64) ;; label = @1
      block ;; label = @2
        local.get 0
        i64.const 255
        i64.and
        i64.const 77
        i64.eq
        if ;; label = @3
          call 45
          if (result i32) ;; label = @4
            local.get 0
            call 44
            local.tee 2
            if (result i32) ;; label = @5
              local.get 2
            else
              local.get 0
              call 9
              drop
              local.get 1
              call 59
              local.get 1
              i32.load
              i32.const 1
              i32.ne
              br_if 3 (;@2;)
              local.get 1
              i32.load offset=4
            end
            i32.const 2
            i32.shl
            i32.load offset=1048708
          else
            i32.const 1
          end
          i32.const 1
          i32.sub
          i64.extend_i32_u
          i64.const 32
          i64.shl
          i64.const 4294967299
          i64.add
          br 2 (;@1;)
        end
        unreachable
      end
      local.get 1
      i64.load offset=8
      local.set 3
      call 62
      i64.const 43519227003201550
      call 63
      local.get 0
      local.get 3
      call 66
      call 8
      drop
      i64.const 2
    end
    local.get 1
    i32.const 16
    i32.add
    global.set 0
  )
  (func (;66;) (type 0) (param i64 i64) (result i64)
    (local i32)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 2
    global.set 0
    local.get 2
    local.get 1
    i64.store offset=8
    local.get 2
    local.get 0
    i64.store
    i32.const 1048896
    i32.const 2
    local.get 2
    i32.const 2
    call 64
    local.get 2
    i32.const 16
    i32.add
    global.set 0
  )
  (func (;67;) (type 20) (param i64 i64 i64 i64 i64) (result i64)
    (local i32 i32 i32 i32 i64 i64)
    global.get 0
    i32.const 96
    i32.sub
    local.tee 5
    global.set 0
    block ;; label = @1
      block ;; label = @2
        block ;; label = @3
          local.get 0
          i64.const 255
          i64.and
          i64.const 77
          i64.ne
          local.get 1
          i64.const 255
          i64.and
          i64.const 77
          i64.ne
          i32.or
          local.get 2
          i64.const 255
          i64.and
          i64.const 75
          i64.ne
          local.get 3
          i64.const 255
          i64.and
          i64.const 77
          i64.ne
          i32.or
          i32.or
          br_if 0 (;@3;)
          local.get 5
          local.get 4
          call 38
          local.get 5
          i32.load
          i32.const 1
          i32.eq
          br_if 0 (;@3;)
          local.get 5
          i64.load offset=24
          local.set 9
          local.get 5
          i64.load offset=16
          local.set 10
          call 45
          i32.eqz
          if ;; label = @4
            i32.const 1
            local.set 6
            local.get 5
            i32.const 1
            i32.store offset=4
            br 3 (;@1;)
          end
          local.get 0
          call 43
          local.tee 6
          if ;; label = @4
            local.get 5
            local.get 6
            i32.store offset=4
            i32.const 1
            local.set 6
            br 3 (;@1;)
          end
          local.get 0
          call 9
          drop
          call 10
          local.set 4
          call 2
          local.get 4
          call 5
          local.get 2
          call 5
          local.get 3
          call 5
          local.get 10
          local.get 9
          call 41
          call 5
          local.get 4
          call 5
          local.set 2
          i32.const 1048694
          i32.const 13
          call 68
          local.set 4
          local.get 5
          call 2
          i64.store offset=32
          local.get 5
          local.get 2
          i64.store offset=24
          local.get 5
          local.get 4
          i64.store offset=16
          local.get 5
          local.get 1
          i64.store offset=8
          local.get 5
          i64.const 0
          i64.store
          local.get 5
          i64.const 2
          i64.store offset=48
          i32.const 0
          local.set 6
          loop ;; label = @4
            local.get 6
            i32.const 40
            i32.ne
            if ;; label = @5
              local.get 5
              i32.const 56
              i32.add
              local.tee 8
              i32.const 1048728
              i32.const 8
              call 50
              local.get 5
              i32.load offset=56
              br_if 2 (;@3;)
              local.get 5
              i64.load offset=64
              local.set 9
              local.get 5
              local.get 5
              local.get 6
              i32.add
              local.tee 7
              i32.const 16
              i32.add
              i64.load
              i64.store offset=72
              local.get 5
              local.get 7
              i32.const 8
              i32.add
              i64.load
              i64.store offset=64
              local.get 5
              local.get 7
              i32.const 24
              i32.add
              i64.load
              i64.store offset=56
              local.get 5
              i32.const 1048932
              i32.const 3
              local.get 8
              i32.const 3
              call 64
              i64.store offset=80
              local.get 5
              local.get 7
              i32.const 32
              i32.add
              i64.load
              i64.store offset=88
              local.get 8
              local.get 9
              i32.const 1048984
              i32.const 2
              local.get 5
              i32.const 80
              i32.add
              i32.const 2
              call 64
              call 52
              local.get 5
              i64.load offset=64
              local.set 9
              local.get 5
              i64.load offset=56
              i64.eqz
              i32.eqz
              br_if 2 (;@3;)
              local.get 5
              local.get 9
              i64.store offset=48
              local.get 6
              i32.const 40
              i32.add
              local.set 6
              br 1 (;@4;)
            end
          end
          i32.const 1
          local.set 6
          local.get 5
          i32.const 48
          i32.add
          i32.const 1
          call 49
          call 11
          drop
          i32.const 6
          local.set 7
          block ;; label = @4
            block ;; label = @5
              local.get 1
              local.get 4
              local.get 2
              call 12
              local.tee 1
              i64.const 255
              i64.and
              i64.const 3
              i64.eq
              br_if 0 (;@5;)
              local.get 5
              local.get 1
              call 38
              local.get 5
              i32.load
              br_if 0 (;@5;)
              local.get 5
              i64.load offset=16
              local.tee 1
              local.get 5
              i64.load offset=24
              local.tee 2
              i64.or
              i64.eqz
              br_if 3 (;@2;)
              local.get 5
              local.get 3
              local.get 1
              local.get 2
              call 31
              local.get 5
              i32.load
              i32.const 1
              i32.ne
              br_if 1 (;@4;)
              local.get 5
              i32.load offset=4
              local.set 7
            end
            local.get 5
            local.get 7
            i32.store offset=4
            br 3 (;@1;)
          end
          i64.const 175127638542
          local.get 0
          call 48
          local.get 5
          local.get 1
          local.get 2
          call 54
          local.get 5
          i32.load
          i32.const 1
          i32.eq
          br_if 0 (;@3;)
          local.get 5
          i64.load offset=8
          local.set 4
          local.get 5
          local.get 3
          i64.store offset=64
          local.get 5
          local.get 4
          i64.store offset=56
          i32.const 1048644
          i32.const 2
          local.get 5
          i32.const 56
          i32.add
          i32.const 2
          call 64
          call 8
          drop
          br 1 (;@2;)
        end
        unreachable
      end
      local.get 5
      local.get 1
      i64.store offset=16
      local.get 5
      local.get 2
      i64.store offset=24
      i32.const 0
      local.set 6
    end
    local.get 5
    local.get 6
    i32.store
    local.get 5
    call 53
    local.get 5
    i32.const 96
    i32.add
    global.set 0
  )
  (func (;68;) (type 12) (param i32 i32) (result i64)
    (local i32 i64)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 2
    global.set 0
    local.get 2
    local.get 0
    local.get 1
    call 82
    local.get 2
    i32.load
    i32.const 1
    i32.eq
    if ;; label = @1
      unreachable
    end
    local.get 2
    i64.load offset=8
    local.get 2
    i32.const 16
    i32.add
    global.set 0
  )
  (func (;69;) (type 6) (param i64 i64 i64 i64) (result i64)
    (local i32 i32 i32 i64 i64 i64 i64 i64)
    global.get 0
    i32.const 48
    i32.sub
    local.tee 4
    global.set 0
    block ;; label = @1
      block ;; label = @2
        block ;; label = @3
          local.get 0
          i64.const 255
          i64.and
          i64.const 77
          i64.ne
          local.get 1
          i64.const 255
          i64.and
          i64.const 77
          i64.ne
          i32.or
          br_if 0 (;@3;)
          local.get 4
          i32.const 16
          i32.add
          local.get 2
          call 38
          local.get 4
          i32.load offset=16
          i32.const 1
          i32.eq
          local.get 3
          i64.const 255
          i64.and
          i64.const 77
          i64.ne
          i32.or
          br_if 0 (;@3;)
          local.get 4
          i64.load offset=40
          local.set 2
          local.get 4
          i64.load offset=32
          local.set 7
          block (result i64) ;; label = @4
            block ;; label = @5
              block ;; label = @6
                call 45
                i32.eqz
                if ;; label = @7
                  i32.const 1
                  local.set 5
                  br 1 (;@6;)
                end
                local.get 0
                call 43
                local.tee 5
                br_if 0 (;@6;)
                local.get 0
                call 9
                drop
                i32.const 4
                local.set 5
                local.get 2
                local.get 7
                i64.or
                i64.eqz
                br_if 0 (;@6;)
                local.get 4
                call 10
                i64.store offset=16
                local.get 4
                i32.const 16
                i32.add
                local.tee 6
                local.get 1
                i64.const 696753673873934
                local.get 6
                i32.const 1
                call 49
                call 13
                call 70
                local.get 4
                i32.load offset=16
                i32.const 1
                i32.eq
                br_if 4 (;@2;)
                local.get 4
                i64.load offset=40
                local.tee 8
                i64.const 0
                i64.lt_s
                br_if 5 (;@1;)
                local.get 4
                i64.load offset=32
                local.get 6
                local.get 1
                call 32
                local.get 4
                i64.load offset=16
                local.tee 0
                local.get 7
                i64.add
                local.tee 11
                local.get 0
                i64.lt_u
                local.tee 6
                local.get 6
                i64.extend_i32_u
                local.get 4
                i64.load offset=24
                local.tee 9
                local.get 2
                i64.add
                i64.add
                local.tee 0
                local.get 9
                i64.lt_u
                local.get 0
                local.get 9
                i64.eq
                select
                br_if 0 (;@6;)
                local.get 11
                i64.lt_u
                local.get 0
                local.get 8
                i64.gt_u
                local.get 0
                local.get 8
                i64.eq
                select
                if ;; label = @7
                  i32.const 6
                  local.set 5
                  br 1 (;@6;)
                end
                local.get 4
                i32.const 16
                i32.add
                local.get 1
                local.get 7
                local.get 2
                call 31
                local.get 4
                i32.load offset=16
                i32.const 1
                i32.ne
                br_if 1 (;@5;)
                local.get 4
                i32.load offset=20
                local.set 5
              end
              local.get 5
              i32.const 1
              i32.sub
              i64.extend_i32_u
              i64.const 32
              i64.shl
              i64.const 4294967299
              i64.add
              br 1 (;@4;)
            end
            i64.const 733055682328846
            local.get 3
            call 48
            local.get 4
            i32.const 16
            i32.add
            local.get 7
            local.get 2
            call 54
            local.get 4
            i32.load offset=16
            i32.const 1
            i32.eq
            br_if 1 (;@3;)
            local.get 4
            i64.load offset=24
            local.set 2
            local.get 4
            local.get 1
            i64.store offset=8
            local.get 4
            local.get 2
            i64.store
            i32.const 1048588
            i32.const 2
            local.get 4
            i32.const 2
            call 64
            call 8
            drop
            i64.const 2
          end
          local.get 4
          i32.const 48
          i32.add
          global.set 0
          return
        end
        unreachable
      end
      unreachable
    end
    i64.const 17179869187
    call 71
    unreachable
  )
  (func (;70;) (type 4) (param i32 i64)
    (local i32 i64)
    local.get 0
    block (result i64) ;; label = @1
      block ;; label = @2
        block ;; label = @3
          local.get 1
          i32.wrap_i64
          i32.const 255
          i32.and
          local.tee 2
          i32.const 69
          i32.ne
          if ;; label = @4
            local.get 2
            i32.const 11
            i32.ne
            br_if 2 (;@2;)
            local.get 0
            local.get 1
            i64.const 63
            i64.shr_s
            i64.store offset=24
            local.get 0
            local.get 1
            i64.const 8
            i64.shr_s
            i64.store offset=16
            br 1 (;@3;)
          end
          local.get 1
          call 24
          local.set 3
          local.get 1
          call 25
          local.set 1
          local.get 0
          local.get 3
          i64.store offset=24
          local.get 0
          local.get 1
          i64.store offset=16
        end
        i64.const 0
        br 1 (;@1;)
      end
      local.get 0
      i64.const 34359740419
      i64.store offset=8
      i64.const 1
    end
    i64.store
  )
  (func (;71;) (type 8) (param i64)
    local.get 0
    call 30
    drop
  )
  (func (;72;) (type 2) (result i64)
    (local i32 i32 i64)
    global.get 0
    i32.const 32
    i32.sub
    local.tee 0
    global.set 0
    local.get 0
    i32.const 16
    i32.add
    call 61
    i32.const 1
    local.set 1
    block ;; label = @1
      local.get 0
      i32.load offset=16
      i32.const 1
      i32.eq
      if ;; label = @2
        local.get 0
        i32.const 1
        i32.store offset=4
        br 1 (;@1;)
      end
      local.get 0
      local.get 0
      i64.load offset=24
      i64.store offset=8
      i32.const 0
      local.set 1
    end
    local.get 0
    local.get 1
    i32.store
    local.get 0
    call 55
    local.get 0
    i32.const 32
    i32.add
    global.set 0
  )
  (func (;73;) (type 2) (result i64)
    (local i32 i32 i32 i32 i64 i64 i64 i64 i64)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 0
    global.set 0
    block ;; label = @1
      block ;; label = @2
        block (result i64) ;; label = @3
          i64.const 1
          i64.const 0
          call 34
          i32.eqz
          if ;; label = @4
            call 2
            br 1 (;@3;)
          end
          i64.const 1
          i64.const 0
          call 35
          local.get 0
          call 39
          i64.const 42949672963
          local.set 4
          local.get 0
          i32.load
          i32.const 1
          i32.ne
          br_if 1 (;@2;)
          local.get 0
          i64.load offset=8
        end
        local.set 5
        call 14
        local.set 4
        local.get 5
        call 3
        i64.const 32
        i64.shr_u
        i32.wrap_i64
        local.set 2
        loop ;; label = @3
          local.get 2
          local.get 3
          local.tee 1
          i32.eq
          br_if 1 (;@2;)
          local.get 1
          i32.const 1
          i32.add
          local.set 3
          local.get 1
          local.get 5
          call 3
          i64.const 32
          i64.shr_u
          i32.wrap_i64
          i32.ge_u
          br_if 0 (;@3;)
          local.get 5
          local.get 1
          i64.extend_i32_u
          i64.const 32
          i64.shl
          i64.const 4
          i64.or
          call 4
          local.tee 6
          i64.const 255
          i64.and
          i64.const 77
          i64.ne
          br_if 2 (;@1;)
          local.get 0
          local.get 6
          call 32
          local.get 0
          i64.load
          local.tee 7
          local.get 0
          i64.load offset=8
          local.tee 8
          i64.or
          i64.eqz
          br_if 0 (;@3;)
          local.get 4
          local.get 6
          local.get 7
          local.get 8
          call 41
          call 15
          local.set 4
          br 0 (;@3;)
        end
        unreachable
      end
      local.get 0
      i32.const 16
      i32.add
      global.set 0
      local.get 4
      return
    end
    unreachable
  )
  (func (;74;) (type 1) (param i64) (result i64)
    (local i32)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 1
    global.set 0
    local.get 0
    i64.const 255
    i64.and
    i64.const 77
    i64.ne
    if ;; label = @1
      unreachable
    end
    local.get 1
    local.get 0
    call 32
    local.get 1
    i64.load
    local.get 1
    i64.load offset=8
    call 41
    local.get 1
    i32.const 16
    i32.add
    global.set 0
  )
  (func (;75;) (type 2) (result i64)
    (local i32 i32 i64)
    global.get 0
    i32.const 32
    i32.sub
    local.tee 0
    global.set 0
    local.get 0
    i32.const 16
    i32.add
    call 59
    i32.const 1
    local.set 1
    block ;; label = @1
      local.get 0
      i32.load offset=16
      i32.const 1
      i32.eq
      if ;; label = @2
        local.get 0
        i32.const 9
        i32.const 8
        local.get 0
        i32.load offset=20
        i32.const 4
        i32.eq
        select
        i32.store offset=4
        br 1 (;@1;)
      end
      local.get 0
      local.get 0
      i64.load offset=24
      i64.store offset=8
      i32.const 0
      local.set 1
    end
    local.get 0
    local.get 1
    i32.store
    local.get 0
    call 55
    local.get 0
    i32.const 32
    i32.add
    global.set 0
  )
  (func (;76;) (type 0) (param i64 i64) (result i64)
    (local i32 i32 i64)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 2
    global.set 0
    block (result i64) ;; label = @1
      block ;; label = @2
        local.get 0
        i64.const 255
        i64.and
        i64.const 77
        i64.ne
        local.get 1
        i64.const 255
        i64.and
        i64.const 77
        i64.ne
        i32.or
        i32.eqz
        if ;; label = @3
          call 45
          if (result i32) ;; label = @4
            local.get 0
            call 44
            local.tee 3
            i32.eqz
            if ;; label = @5
              local.get 0
              call 9
              drop
              local.get 2
              call 59
              local.get 2
              i32.load
              i32.const 1
              i32.eq
              br_if 3 (;@2;)
              local.get 2
              i64.load offset=8
              local.set 4
              i64.const 43519227003201550
              call 63
              local.get 0
              local.get 4
              call 66
              call 8
              drop
              br 3 (;@2;)
            end
            local.get 3
            i32.const 2
            i32.shl
            i32.load offset=1048708
          else
            i32.const 1
          end
          i32.const 1
          i32.sub
          i64.extend_i32_u
          i64.const 32
          i64.shl
          i64.const 4294967299
          i64.add
          br 2 (;@1;)
        end
        unreachable
      end
      i64.const 483902301132814
      local.get 1
      call 77
      call 47
      i64.const 43519227893462286
      call 63
      local.get 2
      local.get 1
      i64.store offset=8
      local.get 2
      local.get 0
      i64.store
      i32.const 1048852
      i32.const 2
      local.get 2
      i32.const 2
      call 64
      call 8
      drop
      i64.const 2
    end
    local.get 2
    i32.const 16
    i32.add
    global.set 0
  )
  (func (;77;) (type 9) (param i64 i64)
    local.get 0
    local.get 1
    i64.const 2
    call 6
    drop
  )
  (func (;78;) (type 1) (param i64) (result i64)
    (local i32 i32 i64)
    global.get 0
    i32.const -64
    i32.add
    local.tee 1
    global.set 0
    block ;; label = @1
      local.get 0
      i64.const 255
      i64.and
      i64.const 77
      i64.eq
      if ;; label = @2
        block ;; label = @3
          call 45
          i32.eqz
          if ;; label = @4
            local.get 1
            i64.const 4294967297
            i64.store
            br 1 (;@3;)
          end
          local.get 1
          i32.const 32
          i32.add
          call 61
          local.get 1
          i32.load offset=32
          i32.const 1
          i32.eq
          if ;; label = @4
            local.get 1
            i64.const 4294967297
            i64.store
            br 1 (;@3;)
          end
          local.get 1
          i64.load offset=40
          call 9
          drop
          call 2
          call 10
          call 5
          local.set 3
          local.get 1
          block (result i32) ;; label = @4
            block ;; label = @5
              block ;; label = @6
                local.get 0
                i32.const 1048687
                i32.const 7
                call 68
                local.get 3
                call 12
                local.tee 3
                i64.const 255
                i64.and
                i64.const 3
                i64.eq
                br_if 0 (;@6;)
                local.get 1
                i32.const 32
                i32.add
                local.get 3
                call 70
                local.get 1
                i64.load offset=32
                local.tee 3
                i64.const 2
                i64.eq
                br_if 0 (;@6;)
                local.get 3
                i32.wrap_i64
                i32.const 1
                i32.and
                i32.eqz
                br_if 1 (;@5;)
              end
              local.get 1
              i32.const 6
              i32.store offset=4
              i32.const 1
              br 1 (;@4;)
            end
            local.get 1
            i64.load offset=56
            local.tee 3
            i64.const 0
            i64.lt_s
            br_if 3 (;@1;)
            local.get 0
            local.get 1
            i64.load offset=48
            local.tee 0
            local.get 3
            call 33
            local.tee 2
            if ;; label = @5
              local.get 1
              i32.const 1
              i32.store
              local.get 1
              local.get 2
              i32.store offset=4
              br 2 (;@3;)
            end
            local.get 1
            local.get 0
            i64.store offset=16
            local.get 1
            local.get 3
            i64.store offset=24
            i32.const 0
          end
          i32.store
        end
        local.get 1
        call 53
        local.get 1
        i32.const -64
        i32.sub
        global.set 0
        return
      end
      unreachable
    end
    i64.const 17179869187
    call 71
    unreachable
  )
  (func (;79;) (type 1) (param i64) (result i64)
    (local i32)
    global.get 0
    i32.const 16
    i32.sub
    local.tee 1
    global.set 0
    block ;; label = @1
      local.get 0
      i64.const 255
      i64.and
      i64.const 72
      i64.ne
      br_if 0 (;@1;)
      local.get 0
      call 16
      i64.const -4294967296
      i64.and
      i64.const 137438953472
      i64.ne
      br_if 0 (;@1;)
      block (result i64) ;; label = @2
        i64.const 4294967299
        call 45
        i32.eqz
        br_if 0 (;@2;)
        drop
        local.get 1
        call 61
        i64.const 12884901891
        local.get 1
        i32.load
        i32.const 1
        i32.eq
        br_if 0 (;@2;)
        drop
        local.get 1
        i64.load offset=8
        call 9
        drop
        local.get 0
        call 17
        drop
        i64.const 2
      end
      local.get 1
      i32.const 16
      i32.add
      global.set 0
      return
    end
    unreachable
  )
  (func (;80;) (type 6) (param i64 i64 i64 i64) (result i64)
    (local i32 i32 i64 i64 i64)
    global.get 0
    i32.const -64
    i32.add
    local.tee 4
    global.set 0
    block (result i64) ;; label = @1
      block ;; label = @2
        block ;; label = @3
          local.get 0
          i64.const 255
          i64.and
          i64.const 77
          i64.ne
          local.get 1
          i64.const 255
          i64.and
          i64.const 77
          i64.ne
          i32.or
          br_if 0 (;@3;)
          local.get 4
          local.get 2
          call 38
          local.get 4
          i32.load
          i32.const 1
          i32.eq
          local.get 3
          i64.const 255
          i64.and
          i64.const 77
          i64.ne
          i32.or
          br_if 0 (;@3;)
          local.get 4
          i64.load offset=24
          local.set 2
          local.get 4
          i64.load offset=16
          local.set 6
          call 45
          i32.eqz
          if ;; label = @4
            i32.const 1
            local.set 5
            br 2 (;@2;)
          end
          local.get 0
          call 43
          local.tee 5
          br_if 1 (;@2;)
          local.get 0
          call 9
          drop
          local.get 2
          local.get 6
          i64.or
          i64.eqz
          if ;; label = @4
            i32.const 4
            local.set 5
            br 2 (;@2;)
          end
          local.get 4
          local.get 1
          call 32
          local.get 4
          i64.load
          local.tee 8
          local.get 6
          i64.lt_u
          local.tee 5
          local.get 4
          i64.load offset=8
          local.tee 7
          local.get 2
          i64.lt_u
          local.get 2
          local.get 7
          i64.eq
          select
          if ;; label = @4
            i32.const 5
            local.set 5
            br 2 (;@2;)
          end
          local.get 1
          local.get 8
          local.get 6
          i64.sub
          local.get 7
          local.get 2
          i64.sub
          local.get 5
          i64.extend_i32_u
          i64.sub
          call 33
          local.tee 5
          br_if 1 (;@2;)
          call 10
          local.set 7
          local.get 2
          i64.const 0
          i64.ge_s
          if ;; label = @4
            local.get 4
            local.get 6
            i64.const 63
            i64.shr_s
            local.get 2
            i64.xor
            i64.const 0
            i64.ne
            local.get 6
            i64.const -36028797018963968
            i64.sub
            i64.const 72057594037927935
            i64.gt_u
            i32.or
            if (result i64) ;; label = @5
              local.get 2
              local.get 6
              call 18
            else
              local.get 6
              i64.const 8
              i64.shl
              i64.const 11
              i64.or
            end
            i64.store offset=56
            local.get 4
            local.get 3
            i64.store offset=48
            local.get 4
            local.get 7
            i64.store offset=40
            i32.const 0
            local.set 5
            loop ;; label = @5
              local.get 5
              i32.const 24
              i32.eq
              if ;; label = @6
                block ;; label = @7
                  i32.const 0
                  local.set 5
                  loop ;; label = @8
                    local.get 5
                    i32.const 24
                    i32.ne
                    if ;; label = @9
                      local.get 4
                      local.get 5
                      i32.add
                      local.get 4
                      i32.const 40
                      i32.add
                      local.get 5
                      i32.add
                      i64.load
                      i64.store
                      local.get 5
                      i32.const 8
                      i32.add
                      local.set 5
                      br 1 (;@8;)
                    end
                  end
                  local.get 1
                  i64.const 65154533130155790
                  local.get 4
                  i32.const 3
                  call 49
                  call 13
                  i64.const 255
                  i64.and
                  i64.const 2
                  i64.ne
                  br_if 0 (;@7;)
                  i64.const 68379099092597774
                  local.get 0
                  call 48
                  local.get 4
                  i32.const 40
                  i32.add
                  local.get 6
                  local.get 2
                  call 54
                  local.get 4
                  i32.load offset=40
                  i32.const 1
                  i32.eq
                  br_if 4 (;@3;)
                  local.get 4
                  i64.load offset=48
                  local.set 2
                  local.get 4
                  local.get 3
                  i64.store offset=16
                  local.get 4
                  local.get 1
                  i64.store offset=8
                  local.get 4
                  local.get 2
                  i64.store
                  i32.const 1048608
                  i32.const 3
                  local.get 4
                  i32.const 3
                  call 64
                  call 8
                  drop
                  i64.const 2
                  br 6 (;@1;)
                end
              else
                local.get 4
                local.get 5
                i32.add
                i64.const 2
                i64.store
                local.get 5
                i32.const 8
                i32.add
                local.set 5
                br 1 (;@5;)
              end
            end
            unreachable
          end
          i64.const 158913789955
          call 71
          unreachable
        end
        unreachable
      end
      local.get 5
      i32.const 1
      i32.sub
      i64.extend_i32_u
      i64.const 32
      i64.shl
      i64.const 4294967299
      i64.add
    end
    local.get 4
    i32.const -64
    i32.sub
    global.set 0
  )
  (func (;81;) (type 4) (param i32 i64)
    block ;; label = @1
      local.get 0
      local.get 1
      i64.const 2
      call 37
      if (result i64) ;; label = @2
        local.get 1
        i64.const 2
        call 0
        local.tee 1
        i64.const 255
        i64.and
        i64.const 77
        i64.ne
        br_if 1 (;@1;)
        local.get 0
        local.get 1
        i64.store offset=8
        i64.const 1
      else
        i64.const 0
      end
      i64.store
      return
    end
    unreachable
  )
  (func (;82;) (type 13) (param i32 i32 i32)
    (local i32 i32 i32 i64)
    block (result i64) ;; label = @1
      block ;; label = @2
        local.get 2
        i32.const 9
        i32.gt_u
        br_if 0 (;@2;)
        local.get 2
        local.set 4
        local.get 1
        local.set 5
        loop ;; label = @3
          local.get 6
          i64.const 8
          i64.shl
          i64.const 14
          i64.or
          local.get 4
          i32.eqz
          br_if 2 (;@1;)
          drop
          block (result i32) ;; label = @4
            i32.const 1
            local.get 5
            i32.load8_u
            local.tee 3
            i32.const 95
            i32.eq
            br_if 0 (;@4;)
            drop
            block ;; label = @5
              local.get 3
              i32.const 48
              i32.sub
              i32.const 255
              i32.and
              i32.const 10
              i32.ge_u
              if ;; label = @6
                local.get 3
                i32.const 65
                i32.sub
                i32.const 255
                i32.and
                i32.const 26
                i32.lt_u
                br_if 1 (;@5;)
                local.get 3
                i32.const 97
                i32.sub
                i32.const 255
                i32.and
                i32.const 26
                i32.ge_u
                br_if 4 (;@2;)
                local.get 3
                i32.const 59
                i32.sub
                br 2 (;@4;)
              end
              local.get 3
              i32.const 46
              i32.sub
              br 1 (;@4;)
            end
            local.get 3
            i32.const 53
            i32.sub
          end
          i64.extend_i32_u
          i64.const 255
          i64.and
          local.get 6
          i64.const 6
          i64.shl
          i64.or
          local.set 6
          local.get 4
          i32.const 1
          i32.sub
          local.set 4
          local.get 5
          i32.const 1
          i32.add
          local.set 5
          br 0 (;@3;)
        end
        unreachable
      end
      local.get 1
      i64.extend_i32_u
      i64.const 32
      i64.shl
      i64.const 4
      i64.or
      local.get 2
      i64.extend_i32_u
      i64.const 32
      i64.shl
      i64.const 4
      i64.or
      call 26
    end
    local.set 6
    local.get 0
    i64.const 0
    i64.store
    local.get 0
    local.get 6
    i64.store offset=8
  )
  (data (;0;) (i32.const 1048576) "amountasset\00\00\00\10\00\06\00\00\00\06\00\10\00\05\00\00\00to\00\00\00\00\10\00\06\00\00\00\06\00\10\00\05\00\00\00\1c\00\10\00\02\00\00\00reward_token\00\00\10\00\06\00\00\008\00\10\00\0c\00\00\00InitializedBalanceAssetListbalanceclaim_rewards\00\00\00\00\00\03\00\00\00\03\00\00\00\08\00\00\00\09\00\00\00ContractCreateContractHostFnCreateContractWithCtorHostFnnew_adminprevious_admin\00\d0\00\10\00\09\00\00\00\d9\00\10\00\0e\00\00\00current_adminpending_admin\00\00\f8\00\10\00\0d\00\00\00\05\01\10\00\0d\00\00\00admincancelled_pending_admin$\01\10\00\05\00\00\00)\01\10\00\17\00\00\00argscontractfn_name\00P\01\10\00\04\00\00\00T\01\10\00\08\00\00\00\5c\01\10\00\07\00\00\00Wasmcontextsub_invocations\00\00\80\01\10\00\07\00\00\00\87\01\10\00\0f\00\00\00executablesalt\00\00\a8\01\10\00\0a\00\00\00\b2\01\10\00\04\00\00\00constructor_args\c8\01\10\00\10\00\00\00\a8\01\10\00\0a\00\00\00\b2\01\10\00\04")
  (@custom "contractspecv0" (after data) "\00\00\00\04\00\00\00\f6Error conditions for treasury contract operations.\0a\0aEach variant represents a specific validation failure or invalid state that\0acan occur during treasury operations. Error codes are used for efficient\0aerror handling and off-chain error reporting.\00\00\00\00\00\00\00\00\00\0dTreasuryError\00\00\00\00\00\00\0a\00\00\00%Contract has not been initialized yet\00\00\00\00\00\00\0eNotInitialized\00\00\00\00\00\01\00\00\005Attempt to initialize an already initialized contract\00\00\00\00\00\00\12AlreadyInitialized\00\00\00\00\00\02\00\00\00%Caller does not have admin privileges\00\00\00\00\00\00\0cUnauthorized\00\00\00\03\00\00\00\19Amount is zero or invalid\00\00\00\00\00\00\0dInvalidAmount\00\00\00\00\00\00\04\00\00\00-Insufficient balance for withdrawal operation\00\00\00\00\00\00\13InsufficientBalance\00\00\00\00\05\00\00\00\1fToken transfer operation failed\00\00\00\00\0eTransferFailed\00\00\00\00\00\06\00\00\00\1cRecipient address is invalid\00\00\00\10InvalidRecipient\00\00\00\07\00\00\00 No pending admin proposal exists\00\00\00\0eNoPendingAdmin\00\00\00\00\00\08\00\00\00\1fCaller is not the pending admin\00\00\00\00\13InvalidPendingAdmin\00\00\00\00\09\00\00\00\1aAsset not found in storage\00\00\00\00\00\0dAssetNotFound\00\00\00\00\00\00\0a\00\00\00\01\00\00\00\00\00\00\00\00\00\00\00\10DepositEventData\00\00\00\02\00\00\00\00\00\00\00\06amount\00\00\00\00\00\0a\00\00\00\00\00\00\00\05asset\00\00\00\00\00\00\13\00\00\00\01\00\00\00\00\00\00\00\00\00\00\00\11WithdrawEventData\00\00\00\00\00\00\03\00\00\00\00\00\00\00\06amount\00\00\00\00\00\0a\00\00\00\00\00\00\00\05asset\00\00\00\00\00\00\13\00\00\00\00\00\00\00\02to\00\00\00\00\00\13\00\00\00\01\00\00\00\00\00\00\00\00\00\00\00\15ClaimRewardsEventData\00\00\00\00\00\00\02\00\00\00\00\00\00\00\06amount\00\00\00\00\00\0a\00\00\00\00\00\00\00\0creward_token\00\00\00\13\00\00\00\02\00\00\00\ffStorage keys for treasury contract.\0a\0aInstance storage is used only for bounded configuration data (initialized flag).\0aDynamic per-asset data (balances) is stored in persistent storage with per-key TTL\0ato avoid size cap issues and shared archival problems.\00\00\00\00\00\00\00\00\0bInstanceKey\00\00\00\00\01\00\00\00\00\00\00\009Flag indicating whether the contract has been initialized\00\00\00\00\00\00\0bInitialized\00\00\00\00\02\00\00\00/Persistent storage keys for per-asset balances.\00\00\00\00\00\00\00\00\0dPersistentKey\00\00\00\00\00\00\02\00\00\00\01\00\00\00\1cBalance for a specific asset\00\00\00\07Balance\00\00\00\00\01\00\00\00\13\00\00\00\00\00\00\001List of assets that have balances (for iteration)\00\00\00\00\00\00\09AssetList\00\00\00\00\00\00\00\00\00\03%Record a deposit of tokens to the treasury.\0a\0aThis function verifies that tokens have been transferred to the treasury contract\0aaddress and updates internal balance tracking. Only the admin can call this function.\0aThe function verifies that the actual\0atoken balance is sufficient to support the claimed deposit amount to prevent fabricated balances.\0a\0a# Arguments\0a* `env` - The execution environment\0a* `caller` - Address of the caller (must be admin)\0a* `asset` - Address of the token contract\0a* `amount` - Amount of tokens being deposited (must be > 0)\0a* `from` - Address that sent the tokens (for event tracking and audit trail)\0a\0a# Returns\0a* `Ok(())` - Deposit recorded successfully\0a* `Err(TreasuryError)` - Deposit failed (unauthorized, not initialized, invalid amount, transfer not verified, or overflow)\00\00\00\00\00\00\07deposit\00\00\00\00\04\00\00\00\00\00\00\00\06caller\00\00\00\00\00\13\00\00\00\00\00\00\00\05asset\00\00\00\00\00\00\13\00\00\00\00\00\00\00\06amount\00\00\00\00\00\0a\00\00\00\00\00\00\00\04from\00\00\00\13\00\00\00\01\00\00\03\e9\00\00\00\02\00\00\07\d0\00\00\00\0dTreasuryError\00\00\00\00\00\00\00\00\00\00TUpgrade the treasury contract WASM.\0aOnly the current admin can authorize an upgrade.\00\00\00\07upgrade\00\00\00\00\01\00\00\00\00\00\00\00\0dnew_wasm_hash\00\00\00\00\00\03\ee\00\00\00 \00\00\00\01\00\00\03\e9\00\00\00\02\00\00\07\d0\00\00\00\0dTreasuryError\00\00\00\00\00\00\00\00\00\03NWithdraw tokens from the treasury (admin only).\0a\0aThis function performs two critical operations atomically: it updates internal\0abalance tracking and transfers tokens to the recipient. If the token transfer\0afails, the balance update is reverted, ensuring consistency. Only authorized\0aadmins (or multisig contract if configured) can perform withdrawals.\0a\0a# Arguments\0a* `env` - The execution environment\0a* `caller` - Admin address (must be authorized - regular admin or multisig contract)\0a* `asset` - Address of the token contract to withdraw\0a* `amount` - Amount of tokens to withdraw (must be > 0 and <= available balance)\0a* `to` - Address to receive the tokens\0a\0a# Returns\0a* `Ok(())` - Withdrawal successful (balance updated and tokens transferred)\0a* `Err(TreasuryError)` - Withdrawal failed (unauthorized, insufficient balance, or transfer failed)\00\00\00\00\00\08withdraw\00\00\00\04\00\00\00\00\00\00\00\06caller\00\00\00\00\00\13\00\00\00\00\00\00\00\05asset\00\00\00\00\00\00\13\00\00\00\00\00\00\00\06amount\00\00\00\00\00\0a\00\00\00\00\00\00\00\02to\00\00\00\00\00\13\00\00\00\01\00\00\03\e9\00\00\00\02\00\00\07\d0\00\00\00\0dTreasuryError\00\00\00\00\00\00\00\00\00\00\8eGet the admin address.\0a\0a# Arguments\0a* `env` - The execution environment\0a\0a# Returns\0a* `Result<Address, TreasuryError>` - Admin address or error\00\00\00\00\00\09get_admin\00\00\00\00\00\00\00\00\00\00\01\00\00\03\e9\00\00\00\13\00\00\07\d0\00\00\00\0dTreasuryError\00\00\00\00\00\00\00\00\00\01\a9Get the balance of a specific asset in the treasury.\0a\0aReturns the internally tracked balance for the given asset. This may differ\0afrom the actual token contract balance if sync_balance() hasn't been called\0aafter direct transfers to the treasury address.\0a\0a# Arguments\0a* `env` - The execution environment\0a* `asset` - Address of the token contract\0a\0a# Returns\0a* `u128` - Balance of the asset (0 if asset has never been deposited)\00\00\00\00\00\00\0bget_balance\00\00\00\00\01\00\00\00\00\00\00\00\05asset\00\00\00\00\00\00\13\00\00\00\01\00\00\00\0a\00\00\00\00\00\00\01tAccept admin role (two-step transfer, step 2).\0aOnly the pending admin can call this to finalize the transfer.\0a\0a# Arguments\0a* `env` - The execution environment\0a* `caller` - Pending admin address (must be authorized)\0a\0a# Returns\0a* `Ok(())` - Admin transfer completed successfully\0a* `Err(TreasuryError)` - Transfer failed (not initialized, no pending admin, or invalid caller)\00\00\00\0caccept_admin\00\00\00\01\00\00\00\00\00\00\00\06caller\00\00\00\00\00\13\00\00\00\01\00\00\03\e9\00\00\00\02\00\00\07\d0\00\00\00\0dTreasuryError\00\00\00\00\00\00\00\00\00\02WSync balance from token contract and update internal tracking.\0a\0aThis function queries the token contract's balance function to reconcile the\0aactual balance held by the treasury with internal tracking. Use this when tokens\0aare transferred directly to the treasury address without calling deposit(), such\0aas when protocol fees are collected automatically.\0a\0a# Arguments\0a* `env` - The execution environment\0a* `asset` - Address of the token contract\0a\0a# Returns\0a* `Ok(u128)` - Current balance synced from token contract\0a* `Err(TreasuryError)` - Sync failed (not initialized or token contract query failed)\00\00\00\00\0csync_balance\00\00\00\01\00\00\00\00\00\00\00\05asset\00\00\00\00\00\00\13\00\00\00\01\00\00\03\e9\00\00\00\0a\00\00\07\d0\00\00\00\0dTreasuryError\00\00\00\00\00\00\00\00\00\01\dcInitialize the treasury contract with an admin address.\0a\0aThe admin address is a single account.\0aOnce initialized, the contract cannot be reinitialized. This ensures the admin\0aaddress remains consistent and prevents accidental reconfiguration.\0a\0a# Arguments\0a* `env` - The execution environment\0a* `admin` - Address of the admin (can be multisig contract for enhanced security)\0a\0a# Returns\0a* `Ok(())` - Treasury initialized successfully\0a* `Err(TreasuryError)` - Construction failed\00\00\00\0d__constructor\00\00\00\00\00\00\01\00\00\00\00\00\00\00\05admin\00\00\00\00\00\00\13\00\00\00\01\00\00\03\e9\00\00\00\02\00\00\07\d0\00\00\00\0dTreasuryError\00\00\00\00\00\00\00\00\00\01?Claim incentives accrued by the treasury contract.\0a\0aThe incentives contract requires the reward owner to authorize claims. Since\0arewards accrue to the treasury address, this admin-only proxy makes the\0atreasury contract call `claim_rewards` for itself and records the claimed\0areward-token balance in treasury accounting.\00\00\00\00\0dclaim_rewards\00\00\00\00\00\00\05\00\00\00\00\00\00\00\06caller\00\00\00\00\00\13\00\00\00\00\00\00\00\0aincentives\00\00\00\00\00\13\00\00\00\00\00\00\00\06assets\00\00\00\00\03\ea\00\00\00\13\00\00\00\00\00\00\00\0creward_token\00\00\00\13\00\00\00\00\00\00\00\06amount\00\00\00\00\00\0a\00\00\00\01\00\00\03\e9\00\00\00\0a\00\00\07\d0\00\00\00\0dTreasuryError\00\00\00\00\00\00\00\00\00\01\cdPropose a new admin address (two-step transfer, step 1).\0aOnly the current admin can propose a new admin.\0aThe proposed admin must call `accept_admin` to complete the transfer.\0a\0a# Arguments\0a* `env` - The execution environment\0a* `caller` - Current admin address (must be authorized)\0a* `pending_admin` - Proposed new admin address\0a\0a# Returns\0a* `Ok(())` - Admin proposal created successfully\0a* `Err(TreasuryError)` - Proposal failed (unauthorized or not initialized)\00\00\00\00\00\00\0dpropose_admin\00\00\00\00\00\00\02\00\00\00\00\00\00\00\06caller\00\00\00\00\00\13\00\00\00\00\00\00\00\0dpending_admin\00\00\00\00\00\00\13\00\00\00\01\00\00\03\e9\00\00\00\02\00\00\07\d0\00\00\00\0dTreasuryError\00\00\00\00\00\00\00\00\00\01>Get all balances in the treasury.\0a\0aReturns a map of all assets that have been deposited to the treasury along\0awith their tracked balances. Assets with zero balance are not included in\0athe map.\0a\0a# Arguments\0a* `env` - The execution environment\0a\0a# Returns\0a* `Map<Address, u128>` - Map of asset addresses to their balances\00\00\00\00\00\10get_all_balances\00\00\00\00\00\00\00\01\00\00\03\e9\00\00\03\ec\00\00\00\13\00\00\00\0a\00\00\07\d0\00\00\00\0dTreasuryError\00\00\00\00\00\00\00\00\00\00\c4Get the pending admin address, if any.\0a\0a# Arguments\0a* `env` - The execution environment\0a\0a# Returns\0a* `Ok(Address)` - Pending admin address\0a* `Err(TreasuryError)` - No pending admin proposal exists\00\00\00\11get_pending_admin\00\00\00\00\00\00\00\00\00\00\01\00\00\03\e9\00\00\00\13\00\00\07\d0\00\00\00\0dTreasuryError\00\00\00\00\00\00\00\00\00\01YCancel a pending admin proposal.\0aOnly the current admin can cancel a pending proposal.\0a\0a# Arguments\0a* `env` - The execution environment\0a* `caller` - Current admin address (must be authorized)\0a\0a# Returns\0a* `Ok(())` - Proposal cancelled successfully\0a* `Err(TreasuryError)` - Cancellation failed (unauthorized, not initialized, or no pending admin)\00\00\00\00\00\00\15cancel_admin_proposal\00\00\00\00\00\00\01\00\00\00\00\00\00\00\06caller\00\00\00\00\00\13\00\00\00\01\00\00\03\e9\00\00\00\02\00\00\07\d0\00\00\00\0dTreasuryError\00\00\00\00\00\00\04\00\00\005Error conditions for upgradeable contract operations.\00\00\00\00\00\00\00\00\00\00\0cUpgradeError\00\00\00\04\00\00\00\00\00\00\00\0cUnauthorized\00\00\00\01\00\00\00\00\00\00\00\0fInvalidWasmHash\00\00\00\00\02\00\00\00\00\00\00\00\0eNoPendingAdmin\00\00\00\00\00\03\00\00\00\00\00\00\00\13InvalidPendingAdmin\00\00\00\00\04\00\00\00\02\00\00\00\22Asset identifier for price queries\00\00\00\00\00\00\00\00\00\05Asset\00\00\00\00\00\00\02\00\00\00\01\00\00\00\00\00\00\00\07Stellar\00\00\00\00\01\00\00\00\13\00\00\00\01\00\00\00\00\00\00\00\05Other\00\00\00\00\00\00\01\00\00\00\11\00\00\00\01\00\00\00\1fPrice data returned from oracle\00\00\00\00\00\00\00\00\09PriceData\00\00\00\00\00\00\02\00\00\00\00\00\00\00\05price\00\00\00\00\00\00\0a\00\00\00\00\00\00\00\09timestamp\00\00\00\00\00\00\06\00\00\00\01\00\00\00!Asset configuration for whitelist\00\00\00\00\00\00\00\00\00\00\0bAssetConfig\00\00\00\00\0a\00\00\00\00\00\00\00\05asset\00\00\00\00\00\07\d0\00\00\00\05Asset\00\00\00\00\00\00QBatch-capable adapter address (any oracle implementing read_price_data interface)\00\00\00\00\00\00\0dbatch_adapter\00\00\00\00\00\03\e8\00\00\00\13\00\00\00\00\00\00\00\0dcustom_oracle\00\00\00\00\00\03\e8\00\00\00\13\00\00\00\00\00\00\00\07enabled\00\00\00\00\01\00\00\009Feed identifier for the batch adapter (e.g. \22BTC\22, \22ETH\22)\00\00\00\00\00\00\07feed_id\00\00\00\03\e8\00\00\00\10\00\00\00\c8DEPRECATED: manual price override removed (feat/remove-manual-price-override).\0aRetained for on-chain storage layout compatibility with existing AssetConfig\0aentries; always None, never read or written.\00\00\00\15manual_override_price\00\00\00\00\00\03\e8\00\00\00\0a\00\00\00]Maximum age in seconds for custom/batch oracle prices (None = use global staleness threshold)\00\00\00\00\00\00\07max_age\00\00\00\03\e8\00\00\00\06\00\00\00[Cached decimals for the oracle source \e2\80\94 skips the decimals() cross-contract call when set\00\00\00\00\0foracle_decimals\00\00\00\03\e8\00\00\00\04\00\00\00<DEPRECATED: retained for storage compatibility; always None.\00\00\00\19override_expiry_timestamp\00\00\00\00\00\03\e8\00\00\00\06\00\00\00<DEPRECATED: retained for storage compatibility; always None.\00\00\00\16override_set_timestamp\00\00\00\00\03\e8\00\00\00\06\00\00\00\01\00\00\00\00\00\00\00\00\00\00\00\0bReserveData\00\00\00\00\0a\00\00\00\00\00\00\00\0fa_token_address\00\00\00\00\13\00\00\00\00\00\00\00\0dconfiguration\00\00\00\00\00\07\d0\00\00\00\14ReserveConfiguration\00\00\00\00\00\00\00\16current_liquidity_rate\00\00\00\00\00\0a\00\00\00\00\00\00\00\1ccurrent_variable_borrow_rate\00\00\00\0a\00\00\00\00\00\00\00\12debt_token_address\00\00\00\00\00\13\00\00\00\00\00\00\00\02id\00\00\00\00\00\04\00\00\00\00\00\00\00\1einterest_rate_strategy_address\00\00\00\00\00\13\00\00\00\00\00\00\00\15last_update_timestamp\00\00\00\00\00\00\06\00\00\00\00\00\00\00\0fliquidity_index\00\00\00\00\0a\00\00\00\00\00\00\00\15variable_borrow_index\00\00\00\00\00\00\0a\00\00\00\01\00\00\00\1dOracle configuration settings\00\00\00\00\00\00\00\00\00\00\0cOracleConfig\00\00\00\07\00\00\00\00\00\00\00\0cbasis_points\00\00\00\0a\00\00\00\00\00\00\00\11conversion_factor\00\00\00\00\00\00\0a\00\00\00\00\00\00\00\0dltv_precision\00\00\00\00\00\00\0a\00\00\00\d3Circuit breaker: max price change between consecutive queries in basis points.\0aDefault: 2000 = 20%. Prevents oracle failures from causing extreme price jumps.\0aSet to 0 to disable. See L-8 security audit finding.\00\00\00\00\14max_price_change_bps\00\00\00\04\00\00\00\00\00\00\00\0fprice_precision\00\00\00\00\04\00\00\00\00\00\00\00\19price_staleness_threshold\00\00\00\00\00\00\06\00\00\00\00\00\00\00\0dwad_precision\00\00\00\00\00\00\04\00\00\00\03\00\00\00\a6Per-reserve policy for positions that use the same asset as collateral and debt.\0aExisting reserves decode to Disabled because the backing bitmap bits default to zero.\00\00\00\00\00\00\00\00\00\0dSameAssetMode\00\00\00\00\00\00\02\00\00\00\00\00\00\00\08Disabled\00\00\00\00\00\00\00\00\00\00\00\07Enabled\00\00\00\00\01\00\00\00\01\00\00\00'Calculated interest rates from strategy\00\00\00\00\00\00\00\00\0fCalculatedRates\00\00\00\00\02\00\00\00\00\00\00\00\0eliquidity_rate\00\00\00\00\00\0a\00\00\00\00\00\00\00\14variable_borrow_rate\00\00\00\0a\00\00\00\01\00\00\00\11User account data\00\00\00\00\00\00\00\00\00\00\0fUserAccountData\00\00\00\00\06\00\00\00\22Available borrows in base currency\00\00\00\00\00\16available_borrows_base\00\00\00\00\00\0a\00\00\00\1dCurrent liquidation threshold\00\00\00\00\00\00\1dcurrent_liquidation_threshold\00\00\00\00\00\00\0a\00\00\00\0dHealth factor\00\00\00\00\00\00\0dhealth_factor\00\00\00\00\00\00\0a\00\00\00\13Loan to value ratio\00\00\00\00\03ltv\00\00\00\00\0a\00\00\00!Total collateral in base currency\00\00\00\00\00\00\15total_collateral_base\00\00\00\00\00\00\0a\00\00\00\1bTotal debt in base currency\00\00\00\00\0ftotal_debt_base\00\00\00\00\0a\00\00\00\01\00\00\00!Reserve initialization parameters\00\00\00\00\00\00\00\00\00\00\11InitReserveParams\00\00\00\00\00\00\09\00\00\00yBorrow cap in whole tokens (e.g., 500000 = 500K tokens)\0aWhen checking caps, multiply by 10^decimals to get smallest units\00\00\00\00\00\00\0aborrow_cap\00\00\00\00\00\0a\00\00\00\1cWhether borrowing is enabled\00\00\00\11borrowing_enabled\00\00\00\00\00\00\01\00\00\00 Number of decimals for the asset\00\00\00\08decimals\00\00\00\04\00\00\00\1fWhether flash loans are enabled\00\00\00\00\11flashloan_enabled\00\00\00\00\00\00\01\00\00\00#Liquidation bonus (in basis points)\00\00\00\00\11liquidation_bonus\00\00\00\00\00\00\04\00\00\00'Liquidation threshold (in basis points)\00\00\00\00\15liquidation_threshold\00\00\00\00\00\00\04\00\00\00%Loan to value ratio (in basis points)\00\00\00\00\00\00\03ltv\00\00\00\00\04\00\00\00 Reserve factor (in basis points)\00\00\00\0ereserve_factor\00\00\00\00\00\04\00\00\00xSupply cap in whole tokens (e.g., 1000000 = 1M tokens)\0aWhen checking caps, multiply by 10^decimals to get smallest units\00\00\00\0asupply_cap\00\00\00\00\00\0a\00\00\00\01\00\00\00DBitmap: each pair of bits = [collateral, borrowed] for reserve index\00\00\00\00\00\00\00\11UserConfiguration\00\00\00\00\00\00\01\00\00\00\00\00\00\00\04data\00\00\00\0a\00\00\00\01\00\00\01\00Bitmap layout:\0adata_low: LTV (0-13), liquidation_threshold (14-27), liquidation_bonus (28-41),\0adecimals (42-49), flags (50-56), reserve_factor (57-70),\0amin_remaining_debt (71-102), same_asset_mode (103-104)\0adata_high: borrow_cap (0-63), supply_cap (64-127)\00\00\00\00\00\00\00\14ReserveConfiguration\00\00\00\02\00\00\00\00\00\00\00\09data_high\00\00\00\00\00\00\0a\00\00\00\00\00\00\00\08data_low\00\00\00\0a\00\00\00\01\00\00\00[Flash liquidation validation parameters\0aPassed to helper contract to reduce parameter count\00\00\00\00\00\00\00\00 FlashLiquidationValidationParams\00\00\00\0f\00\00\00\00\00\00\00\10close_factor_bps\00\00\00\0a\00\00\00\00\00\00\00\10collateral_asset\00\00\00\13\00\00\00\00\00\00\00\10collateral_price\00\00\00\0a\00\00\00\00\00\00\00\12collateral_reserve\00\00\00\00\07\d0\00\00\00\0bReserveData\00\00\00\00\00\00\00\00\13collateral_to_seize\00\00\00\00\0a\00\00\00\00\00\00\00\0adebt_asset\00\00\00\00\00\13\00\00\00\00\00\00\00\0cdebt_balance\00\00\00\0a\00\00\00\00\00\00\00\0adebt_price\00\00\00\00\00\0a\00\00\00\00\00\00\00\0cdebt_reserve\00\00\07\d0\00\00\00\0bReserveData\00\00\00\00\00\00\00\00\0ddebt_to_cover\00\00\00\00\00\00\0a\00\00\00\00\00\00\00\0emin_output_bps\00\00\00\00\00\0a\00\00\00\00\00\00\00\0cmin_swap_out\00\00\00\0a\00\00\00\00\00\00\00\16oracle_price_precision\00\00\00\00\00\04\00\00\00\00\00\00\00\06router\00\00\00\00\00\13\00\00\00\00\00\00\00\04user\00\00\00\13\00\00\00\01\00\00\00UFlash liquidation validation result\0aReturned by the flash liquidation helper contract\00\00\00\00\00\00\00\00\00\00 FlashLiquidationValidationResult\00\00\00\05\00\00\00\00\00\00\00\1acollateral_amount_to_seize\00\00\00\00\00\0a\00\00\00\00\00\00\00\12debt_to_cover_base\00\00\00\00\00\0a\00\00\00\00\00\00\00\11effective_min_out\00\00\00\00\00\00\0a\00\00\00\00\00\00\00\11expected_debt_out\00\00\00\00\00\00\0a\00\00\00\00\00\00\00\0ftotal_debt_base\00\00\00\00\0a\00\00\00\04\00\00\00\00\00\00\00\00\00\00\00\0aTokenError\00\00\00\00\00\0c\00\00\00\00\00\00\00\13InsufficientBalance\00\00\00\00\01\00\00\00\00\00\00\00\0eTransferFailed\00\00\00\00\00\02\00\00\00\00\00\00\00\0aMintFailed\00\00\00\00\00\03\00\00\00\00\00\00\00\0aBurnFailed\00\00\00\00\00\04\00\00\00\00\00\00\00\10InvalidRecipient\00\00\00\05\00\00\00\00\00\00\00\0dTokenNotFound\00\00\00\00\00\00\06\00\00\00\00\00\00\00\0cUnauthorized\00\00\00\07\00\00\00\00\00\00\00\0dInvalidAmount\00\00\00\00\00\00\08\00\00\00\00\00\00\00\15InsufficientAllowance\00\00\00\00\00\00\09\00\00\00\00\00\00\00\0cInvalidIndex\00\00\00\0a\00\00\00\00\00\00\00\14UnsupportedOperation\00\00\00\0b\00\00\00\00\00\00\00\12AlreadyInitialized\00\00\00\00\00\0c\00\00\00\04\00\00\00\00\00\00\00\00\00\00\00\0bOracleError\00\00\00\00\13\00\00\00\00\00\00\00\12AssetPriceNotFound\00\00\00\00\00\01\00\00\00\00\00\00\00\11PriceSourceNotSet\00\00\00\00\00\00\02\00\00\00\00\00\00\00\12InvalidPriceSource\00\00\00\00\00\03\00\00\00\00\00\00\00\0bPriceTooOld\00\00\00\00\04\00\00\00\00\00\00\00\16PriceHeartbeatExceeded\00\00\00\00\00\05\00\00\00\00\00\00\00\0eNotInitialized\00\00\00\00\00\06\00\00\00\00\00\00\00\13AssetNotWhitelisted\00\00\00\00\07\00\00\00\00\00\00\00\0dAssetDisabled\00\00\00\00\00\00\08\00\00\00\00\00\00\00\11OracleQueryFailed\00\00\00\00\00\00\09\00\00\00\00\00\00\00\12InvalidCalculation\00\00\00\00\00\0a\00\00\00\00\00\00\00\16FallbackNotImplemented\00\00\00\00\00\0b\00\00\00\00\00\00\00\12AlreadyInitialized\00\00\00\00\00\0c\00\00\00\00\00\00\00\17AssetAlreadyWhitelisted\00\00\00\00\0d\00\00\00\00\00\00\00\0cUnauthorized\00\00\00\0e\00\00\00\00\00\00\00\19PriceManipulationDetected\00\00\00\00\00\00\0f\00\00\00\00\00\00\00\13PriceChangeTooLarge\00\00\00\00\10\00\00\00\00\00\00\00\0cMathOverflow\00\00\00\12\00\00\00\00\00\00\00\0cInvalidPrice\00\00\00\13\00\00\00\04M-05\00\00\00\0dInvalidConfig\00\00\00\00\00\00\14\00\00\00\04\00\00\00\00\00\00\00\00\00\00\00\0dSecurityError\00\00\00\00\00\00\03\00\00\00\00\00\00\00\12ReentrancyDetected\00\00\00\00\00\01\00\00\00\00\00\00\00\14InvalidFundingAmount\00\00\00\02\00\00\00\00\00\00\00\12TTLExtensionFailed\00\00\00\00\00\03\00\00\00\04\00\00\00\00\00\00\00\00\00\00\00\0eOperationError\00\00\00\00\00\06\00\00\00\00\00\00\00\10InvalidRecipient\00\00\00\01\00\00\00\00\00\00\00\11RecipientIsAToken\00\00\00\00\00\00\02\00\00\00\00\00\00\00\14RecipientIsDebtToken\00\00\00\03\00\00\00\00\00\00\00\13DebtTokenBurnFailed\00\00\00\00\04\00\00\00\00\00\00\00\12InvalidRepayAmount\00\00\00\00\00\05\00\00\00CLOW-3: Partial repay would leave dust debt below min_remaining_debt\00\00\00\00\13RepayWouldLeaveDust\00\00\00\00\06\00\00\00\04\00\00\00\00\00\00\00\00\00\00\00\10UserReserveError\00\00\00\01\00\00\00\00\00\00\00\17MaxUserReservesExceeded\00\00\00\00\01\00\00\00\04\00\00\00<L-13\0aReplaces raw panic!() calls for on-chain debuggability.\00\00\00\00\00\00\00\12ConfigurationError\00\00\00\00\00\03\00\00\00\1cLTV exceeds 10000 bps (100%)\00\00\00\0aInvalidLTV\00\00\00\00\00\01\00\00\00.Liquidation threshold exceeds 10000 bps (100%)\00\00\00\00\00\1bInvalidLiquidationThreshold\00\00\00\00\02\00\00\00*Liquidation bonus exceeds 10000 bps (100%)\00\00\00\00\00\17InvalidLiquidationBonus\00\00\00\00\03\00\00\00\04\00\00\00\00\00\00\00\00\00\00\00\12KineticRouterError\00\00\00\00\007\00\00\00\00\00\00\00\0dInvalidAmount\00\00\00\00\00\00\01\00\00\00\00\00\00\00\0eAssetNotActive\00\00\00\00\00\02\00\00\00\00\00\00\00\0bAssetFrozen\00\00\00\00\03\00\00\00\00\00\00\00\0bAssetPaused\00\00\00\00\04\00\00\00\00\00\00\00\13BorrowingNotEnabled\00\00\00\00\05\00\00\00\00\00\00\00\16InsufficientCollateral\00\00\00\00\00\07\00\00\00\00\00\00\00\12HealthFactorTooLow\00\00\00\00\00\08\00\00\00\00\00\00\00\13PriceOracleNotFound\00\00\00\00\0a\00\00\00\00\00\00\00\12InvalidLiquidation\00\00\00\00\00\0b\00\00\00\00\00\00\00\18LiquidationAmountTooHigh\00\00\00\0c\00\00\00\00\00\00\00\15NoDebtOfRequestedType\00\00\00\00\00\00\0d\00\00\00\00\00\00\00\16InvalidFlashLoanParams\00\00\00\00\00\0e\00\00\00\00\00\00\00\16FlashLoanNotAuthorized\00\00\00\00\00\0f\00\00\00\00\00\00\00\16IsolationModeViolation\00\00\00\00\00\10\00\00\00\00\00\00\00\1bPriceOracleInvocationFailed\00\00\00\00\11\00\00\00\00\00\00\00\10PriceOracleError\00\00\00\12\00\00\00\00\00\00\00\11SupplyCapExceeded\00\00\00\00\00\00\13\00\00\00\00\00\00\00\11BorrowCapExceeded\00\00\00\00\00\00\14\00\00\00\00\00\00\00\13DebtCeilingExceeded\00\00\00\00\15\00\00\00\00\00\00\00\13UserInIsolationMode\00\00\00\00\16\00\00\00\00\00\00\00\0fReserveNotFound\00\00\00\00\18\00\00\00\00\00\00\00\0cUserNotFound\00\00\00\19\00\00\00\00\00\00\00\0cUnauthorized\00\00\00\1a\00\00\00\00\00\00\00\12AlreadyInitialized\00\00\00\00\00\1b\00\00\00\00\00\00\00\0eNotInitialized\00\00\00\00\00\1c\00\00\00\00\00\00\00\19ReserveAlreadyInitialized\00\00\00\00\00\00\1d\00\00\00\00\00\00\00\18FlashLoanExecutionFailed\00\00\00\1e\00\00\00\00\00\00\00\12FlashLoanNotRepaid\00\00\00\00\00\1f\00\00\00\00\00\00\00\1eInsufficientFlashLoanLiquidity\00\00\00\00\00 \00\00\00\00\00\00\00\10ATokenMintFailed\00\00\00!\00\00\00\00\00\00\00\13DebtTokenMintFailed\00\00\00\00\22\00\00\00\00\00\00\00\18UnderlyingTransferFailed\00\00\00#\00\00\00\00\00\00\00\17FlashLoanTransferFailed\00\00\00\00$\00\00\00\00\00\00\00\0cMathOverflow\00\00\00%\00\00\00\00\00\00\00\07Expired\00\00\00\00&\00\00\00\00\00\00\00\13InsufficientSwapOut\00\00\00\00'\00\00\00\00\00\00\00\0fMinProfitNotMet\00\00\00\00(\00\00\00\00\00\00\00\0eTreasuryNotSet\00\00\00\00\00)\00\00\00\00\00\00\00\15InsufficientLiquidity\00\00\00\00\00\00*\00\00\00\00\00\00\00\0bAMMRequired\00\00\00\00+\00\00\00\00\00\00\00\0fUnauthorizedAMM\00\00\00\00,\00\00\00\00\00\00\00\15AdapterNotInitialized\00\00\00\00\00\00-\00\00\00\00\00\00\00\10ATokenBurnFailed\00\00\00.\00\00\00\00\00\00\00\0eWASMHashNotSet\00\00\00\00\00/\00\00\00\00\00\00\00\15TokenDeploymentFailed\00\00\00\00\00\000\00\00\00\00\00\00\00\19TokenInitializationFailed\00\00\00\00\00\001\00\00\00\00\00\00\00\15AddressNotWhitelisted\00\00\00\00\00\002\00\00\00\00\00\00\00\0eNoPendingAdmin\00\00\00\00\003\00\00\00\00\00\00\00\13InvalidPendingAdmin\00\00\00\004\00\00\00\00\00\00\00\0fTokenCallFailed\00\00\00\005\00\00\00\00\00\00\00\1dCannotDeactivateActiveReserve\00\00\00\00\00\008\00\00\00\00\00\00\00\1aArchivedInterestRateParams\00\00\00\00\009\00\00\00\00\00\00\00\1bSameAssetPositionNotEnabled\00\00\00\00:\00\00\00\00\00\00\00\1aStrategyCheckpointRequired\00\00\00\00\00;\00\00\00\00\00\00\00\1aStrategyHistoryUnavailable\00\00\00\00\00<\00\00\00\04\00\00\00\00\00\00\00\00\00\00\00\16ReserveManagementError\00\00\00\00\00\02\00\00\00\00\00\00\00\12MaxReservesReached\00\00\00\00\00\01\00\00\00\00\00\00\00\17CannotDropActiveReserve\00\00\00\00\02\00\00\00\01\00\00\00\00\00\00\00\00\00\00\00\0aRepayEvent\00\00\00\00\00\05\00\00\00\00\00\00\00\06amount\00\00\00\00\00\0a\00\00\00\00\00\00\00\07repayer\00\00\00\00\13\00\00\00\00\00\00\00\07reserve\00\00\00\00\13\00\00\00\00\00\00\00\0cuse_a_tokens\00\00\00\01\00\00\00\00\00\00\00\04user\00\00\00\13\00\00\00\01\00\00\00\00\00\00\00\00\00\00\00\0bBorrowEvent\00\00\00\00\07\00\00\00\00\00\00\00\06amount\00\00\00\00\00\0a\00\00\00\00\00\00\00\0bborrow_rate\00\00\00\00\0a\00\00\00\00\00\00\00\10borrow_rate_mode\00\00\00\04\00\00\00\00\00\00\00\0con_behalf_of\00\00\00\13\00\00\00\00\00\00\00\0dreferral_code\00\00\00\00\00\00\04\00\00\00\00\00\00\00\07reserve\00\00\00\00\13\00\00\00\00\00\00\00\04user\00\00\00\13\00\00\00\01\00\00\00\00\00\00\00\00\00\00\00\0bSupplyEvent\00\00\00\00\05\00\00\00\00\00\00\00\06amount\00\00\00\00\00\0a\00\00\00\00\00\00\00\0con_behalf_of\00\00\00\13\00\00\00\00\00\00\00\0dreferral_code\00\00\00\00\00\00\04\00\00\00\00\00\00\00\07reserve\00\00\00\00\13\00\00\00\00\00\00\00\04user\00\00\00\13\00\00\00\01\00\00\00\00\00\00\00\00\00\00\00\0dWithdrawEvent\00\00\00\00\00\00\04\00\00\00\00\00\00\00\06amount\00\00\00\00\00\0a\00\00\00\00\00\00\00\07reserve\00\00\00\00\13\00\00\00\00\00\00\00\02to\00\00\00\00\00\13\00\00\00\00\00\00\00\04user\00\00\00\13\00\00\00\01\00\00\00\00\00\00\00\00\00\00\00\12AdminAcceptedEvent\00\00\00\00\00\02\00\00\00\00\00\00\00\09new_admin\00\00\00\00\00\00\13\00\00\00\00\00\00\00\0eprevious_admin\00\00\00\00\00\13\00\00\00\01\00\00\00\00\00\00\00\00\00\00\00\12AdminProposedEvent\00\00\00\00\00\02\00\00\00\00\00\00\00\0dcurrent_admin\00\00\00\00\00\00\13\00\00\00\00\00\00\00\0dpending_admin\00\00\00\00\00\00\13\00\00\00\01\00\00\00\00\00\00\00\00\00\00\00\14LiquidationCallEvent\00\00\00\09\00\00\00\00\00\00\00\10collateral_asset\00\00\00\13\00\00\00\00\00\00\00\0adebt_asset\00\00\00\00\00\13\00\00\00\00\00\00\00\0ddebt_to_cover\00\00\00\00\00\00\0a\00\00\00\00\00\00\00\1cliquidated_collateral_amount\00\00\00\0a\00\00\00\00\00\00\00\0aliquidator\00\00\00\00\00\13\00\00\00\00\00\00\00\15liquidator_collateral\00\00\00\00\00\00\0a\00\00\00\00\00\00\00\0cprotocol_fee\00\00\00\0a\00\00\00\00\00\00\00\0freceive_a_token\00\00\00\00\01\00\00\00\00\00\00\00\04user\00\00\00\13\00\00\00\01\00\00\00\00\00\00\00\00\00\00\00\1bAdminProposalCancelledEvent\00\00\00\00\02\00\00\00\00\00\00\00\05admin\00\00\00\00\00\00\13\00\00\00\00\00\00\00\17cancelled_pending_admin\00\00\00\00\13")
  (@custom "contractenvmetav0" (after data) "\00\00\00\00\00\00\00\19\00\00\00\00")
  (@custom "contractmetav0" (after data) "\00\00\00\00\00\00\00\05rsver\00\00\00\00\00\00\061.94.1\00\00\00\00\00\00\00\00\00\08rssdkver\00\00\00/25.3.1#e50d95af029c83196dd122f0154bac3f1302394b\00")
  (@custom "contractmetav0" (after data) "\00\00\00\00\00\00\00\06cliver\00\00\00\00\00/25.2.0#28484880988199233a7e8e87c97cb12dac323cb3\00")
)
